	...
	public void editarCliente(String rutCliente, int tipoDato, String nuevoDato) {
		...
		case 0:{
			if (nuevoDato.equals("Activo")) {
				...
				lista.stream().filter(cliente-> cliente.getRunCliente().equals(rutCliente)).forEach(cliente-> cliente.setNombreCategoria(CategoriaEnum.ACTIVO));
				
			}else if(nuevoDato.equals("Inactivo")) {

				lista.stream().filter(cliente-> cliente.getRunCliente().equals(rutCliente)).forEach(cliente-> cliente.setNombreCategoria(CategoriaEnum.INACTIVO));
